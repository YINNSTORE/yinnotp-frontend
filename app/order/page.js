...
import { ArrowLeft, RefreshCw, SignalHigh, SignalLow, Search, ChevronRight, X, ChevronLeft, Bell, BellOff, Copy, Clock, Mail, ShoppingBag, XCircle, Wallet, ExternalLink, Sparkles, } from "lucide-react";
import { ping, roCountries, roOrder, roServices, roOperators, roStatusGet, roStatusSet, } from "../_lib/rumahotpClient";
...
function MsBadge({ ms }) {
    ...
    return (<span className={cx("inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-extrabold", cls)} title="Response server">
      {msLabel(ms)}
    </span>);
}
...
function errMsg(defaultMsg, r) {
    ...
    if (type.includes("balance") ||
        ...
        if (need || bal) {
            return `Saldo tidak cukup. Butuh ${formatIDR(need)}, saldo ${formatIDR(bal)}`;
        }
...
function flagCodeForCountry(country) {
    ...
    if (iso2)
        return iso2;
    if (nameNorm.includes("indonesia"))
        return "id";
    if (nameNorm.includes("philippines"))
        return "ph";
    if (nameNorm.includes("malaysia"))
        return "my";
    if (nameNorm.includes("thailand"))
        return "th";
    if (nameNorm.includes("colombia"))
        return "co";
    return "";
}
function flagUrlFromCountry(country) {
    const direct = String(country?.flag_img ||
        country?.flag_url ||
        country?.img ||
        country?.image ||
        "").trim();
...
function Reveal({ children, className = "" }) {
    ...
    return (<div ref={ref} className={cx("will-change-transform", className)} style={{
            opacity: inView ? 1 : 0,
...
function BootPageSkeleton() {
    return (<div className="mx-auto max-w-[520px] px-4 pt-4 pb-[calc(120px+env(safe-area-inset-bottom))]">
...
function FullscreenBoot({ show }) {
    if (!show)
        return null;
    return (<div className="fixed inset-0 z-[90] bg-[var(--yinn-bg)]">
      <BootPageSkeleton />
    </div>);
}
      ...
      <div className="absolute bottom-0 left-0 right-0 mx-auto w-full max-w-[520px] rounded-t-3xl border border-[var(--yinn-border)] bg-[var(--yinn-surface)] shadow-[0_-20px_60px_rgba(0,0,0,0.25)]">
        <div className="p-4">
          ...
          <div className="mb-3 flex items-start justify-between gap-3">
            ...
            <button className="grid h-10 w-10 place-items-center rounded-xl border border-[var(--yinn-border)]" onClick={onClose} aria-label="Tutup" title="Tutup">
              <X size={18}/>
            </button>
          </div>
        </div>

        <div className="px-4 pb-[calc(16px+env(safe-area-inset-bottom))] yinn-smoothscroll" style={{
            maxHeight: "72vh",
            overflowY: "auto",
...
export default function OrderPage() {
    ...
    function kickModalLoading(ms = 450) {
        ...
        if (modalKickRef.current)
            clearTimeout(modalKickRef.current);
    ...
    const popularResolved = useMemo(() => {
        ...
        for (const s of services)
            map.set(normalizeName(s?.service_name), s);
    ...
    const pickedServiceLogo = useMemo(() => {
        if (pickedService?.service_img)
            return pickedService.service_img;
    ...
    function isWalletSuccess(r) {
        ...
        if (!j)
            return false;
        const flag = j.success ?? j.status ?? j.ok ?? j.result;
    ...
    async function startPolling(order_id) {
        ...
        pollRef.current = setInterval(async () => {
            ...
            if ((st.includes("cancel") || st.includes("canceled")) &&
    ...
    useEffect(() => {
        if (activeOrder?.order_id)
            startPolling(activeOrder.order_id);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    return (<div className="min-h-screen bg-[var(--yinn-bg)] text-[var(--yinn-text)]">
      ...
      <header className="sticky top-0 z-40 border-b border-[var(--yinn-border)] bg-[var(--yinn-surface)]">
        <div className="mx-auto flex max-w-[520px] items-center gap-3 px-4 py-3">
          <Link href="/dashboard" className="grid h-10 w-10 place-items-center rounded-xl border border-[var(--yinn-border)]" style={{ boxShadow: "var(--yinn-soft)" }} aria-label="Kembali" title="Kembali">
            <ArrowLeft size={18}/>
          </Link>
          ...
          <div className="ms-auto flex items-center gap-2">
            <button onClick={refreshPing} className="grid h-10 w-10 place-items-center rounded-xl border border-[var(--yinn-border)]" aria-label="Refresh ping" title="Refresh">
      ...
      <main className="mx-auto max-w-[520px] px-4 pt-4 pb-[calc(120px+env(safe-area-inset-bottom))]">
        ...
        <section className="grid grid-cols-2 gap-3">
          ...
          <div className="rounded-2xl border p-4" style={{
            ...
            <div className="flex items-center gap-2">
              <div className={cx("inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-xs font-extrabold", online
            ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-600"
            : "border-zinc-500/30 bg-zinc-500/10 text-zinc-600")}>
              ...
              <button onClick={toggleNotif} className={cx("ms-auto inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-xs font-extrabold", notifEnabled
            ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-600"
            : "border-zinc-500/30 bg-zinc-500/10 text-zinc-600", notifState === "unsupported" ? "opacity-60" : "")} title="Toggle notifikasi" aria-label="Toggle notifikasi" disabled={notifState === "unsupported"}>
            ...
            <button onClick={openBuyModal} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-2xl py-3 text-sm font-extrabold text-white" style={{
            ...
            <div className="mt-3 rounded-2xl border p-3" style={{
            borderColor: "var(--yinn-border)",
            boxShadow: "var(--yinn-soft)",
            background: "var(--yinn-surface)",
        }}>
              <div className="flex items-center gap-3">
                <div className="grid h-12 w-12 place-items-center rounded-full text-white" style={{
            ...
            color: "rgb(22,163,74)",
              ...
              <button onClick={() => loadBalance({ silent: false })} className="mt-2 w-full rounded-xl border border-[var(--yinn-border)] py-2 text-xs font-extrabold text-[var(--yinn-muted)]">
                Refresh Saldo
              </button>
            </div>
          </div>
          ...
          {/* kanan promo */}
          <div className="rounded-2xl border p-4 text-white" style={{
            borderColor: "var(--yinn-border)",
            boxShadow: "var(--yinn-soft)",
            background: "linear-gradient(135deg, rgba(255,122,0,0.95), rgba(155,81,224,0.95))",
        }}>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              {popularResolved.slice(0, 5).map((p) => (<div key={p.name} className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1 text-xs font-extrabold">
                  <img src={p.img} alt={p.name} className="h-4 w-4 rounded-sm" loading="lazy"/>
                  {p.name}
                </div>))}
            </div>

            <button onClick={openBuyModal} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-white/15 py-3 text-sm font-extrabold">
              Beli Nomor
              <ChevronRight size={18}/>
            </button>
          </div>
        ...
        <section className="mt-4 rounded-2xl border p-4" style={{
            background: "var(--yinn-surface)",
            borderColor: "var(--yinn-border)",
            boxShadow: "var(--yinn-soft)",
        }}>
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => activeOrder?.order_id && startPolling(activeOrder.order_id)} className="grid h-10 w-14 place-items-center rounded-xl border border-[var(--yinn-border)]" aria-label="Sync order" title="Sync">
          ...
          {activeOrder ? ((() => {
            ...
            return (<div className="mt-3 rounded-2xl border border-[var(--yinn-border)] bg-white/40 p-4 dark:bg-black/10">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-3">
                        <div className="grid h-8 w-10 place-items-center">
                          {flagUrl ? (<img src={flagUrl} alt="flag" className="h-6 w-8 rounded-sm object-cover"/>) : (<div className="h-6 w-8 rounded-sm bg-black/10 dark:bg-white/10"/>)}
                        </div>
                ...
                }} className="grid h-9 w-9 place-items-center rounded-xl border border-[var(--yinn-border)] bg-white/60 dark:bg-black/10" title="Copy" aria-label="Copy nomor">
                  ...
                  <div className="mt-4 rounded-2xl border border-[var(--yinn-border)] bg-[var(--yinn-surface)] p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex min-w-0 items-center gap-3">
                        <div className="grid h-12 w-12 place-items-center overflow-hidden rounded-2xl border border-[var(--yinn-border)] bg-black/5 dark:bg-white/5">
                          {appImg ? (<img src={appImg} alt={appName} className="h-9 w-9" loading="lazy"/>) : (<div className="text-xs font-extrabold">APP</div>)}
                        </div>
                    ...
                    <div className="mt-2 text-sm text-[var(--yinn-muted)]">
                      {canCancel ? (<span>Sudah bisa klik batal.</span>) : (<span>
                          Tunggu{" "}
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    <button onClick={openBuyModal} className="inline-flex items-center justify-center gap-2 rounded-2xl border py-3 text-sm font-extrabold" style={{
                    ...
                    <button onClick={() => setStatus("cancel")} disabled={!canCancel} className="inline-flex items-center justify-center gap-2 rounded-2xl border py-3 text-sm font-extrabold disabled:opacity-50" style={{
              ...
              <button onClick={openBuyModal} className="mt-4 inline-flex items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-extrabold text-white" style={{
                background: "linear-gradient(135deg, var(--yinn-brand-from), var(--yinn-brand-to))",
            }}>
                Buat Pesanan
              </button>
        ...
        <section className="mt-4 grid grid-cols-2 gap-3">
          <div className="rounded-2xl border p-4" style={{
            ...
            <div className="mt-3 flex items-center gap-2">
              <div className={cx("rounded-full border px-3 py-1 text-xs font-extrabold", notifEnabled
            ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-600"
            : "border-zinc-500/30 bg-zinc-500/10 text-zinc-600")}>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <button className="rounded-xl border border-[var(--yinn-border)] py-2 text-sm font-extrabold" onClick={() => {
            updateNotifStateFromBrowser();
            if (notifState === "unsupported")
                toast.error("Browser tidak support notifikasi");
            else
                toast.success(`Status: ${typeof Notification !== "undefined"
                    ? Notification.permission
                    : notifState}`);
        }}>
                Cek
              </button>
              <button className="rounded-xl py-2 text-sm font-extrabold text-white disabled:opacity-60" style={{
            background: "linear-gradient(135deg, var(--yinn-brand-from), var(--yinn-brand-to))",
        }} onClick={toggleNotif} disabled={notifState === "unsupported"}>
      ...
      {/* BUY MODAL */}
      <Modal open={openBuy} onClose={() => setOpenBuy(false)} title="Beli Nomor Virtual" subtitle="Pilih aplikasi lalu negara & harga">
        <div className="mb-3 flex items-center justify-between">
          ...
          {buyStep === "country" ? (<button onClick={() => {
                setBuyStep("app");
                setCountrySearch("");
                setExpandedCountryId("");
                kickModalLoading();
        ...
        {buyStep === "app" ? (<>
            <div className="flex items-center gap-2 rounded-2xl border border-[var(--yinn-border)] px-3 py-2">
              ...
              <input value={serviceSearch} onChange={(e) => setServiceSearch(e.target.value)} placeholder="Cari nama aplikasi..." className="w-full bg-transparent py-2 text-sm outline-none" disabled={showAppLoading}/>
            </div>
            ...
            {showAppLoading ? (<div className="mt-2 grid grid-cols-3 gap-2">
                {Array.from({ length: 6 }).map((_, i) => (<div key={i} className="rounded-2xl border border-[var(--yinn-border)] p-3">
              ...
              </div>) : (<div className="mt-2 grid grid-cols-3 gap-2">
                {popularResolved.map((p) => (<Reveal key={String(p.svc?.service_code)}>
                    <button onClick={() => {
            ...
            <div className="mt-3 overflow-hidden rounded-2xl border border-[var(--yinn-border)]">
              {showAppLoading ? (<div className="divide-y divide-[var(--yinn-border)]">
                  {Array.from({ length: 8 }).map((_, i) => (<SkeletonRowApp key={i}/>))}
                </div>) : (<div className="divide-y divide-[var(--yinn-border)]">
                  {filteredServices.map((s) => (<Reveal key={String(s?.service_code)}>
                      <button onClick={() => {
                  ...
                  {!filteredServices.length && (<div className="p-4 text-sm text-[var(--yinn-muted)]">
            ...
            <div className="rounded-2xl border border-[var(--yinn-border)] p-3">
              <div className="flex items-center gap-3">
                <div className="grid h-12 w-12 place-items-center overflow-hidden rounded-2xl border border-[var(--yinn-border)] bg-black/5 dark:bg-white/5">
                  {pickedServiceLogo ? (<img src={pickedServiceLogo} alt={pickedService?.service_name || "App"} className="h-9 w-9" loading="lazy"/>) : (<div className="text-xs font-extrabold">APP</div>)}
                </div>
            ...
            <div className="mt-3 flex items-center gap-2 rounded-2xl border border-[var(--yinn-border)] px-3 py-2">
              <Search size={16} className="text-[var(--yinn-muted)]"/>
              <input value={countrySearch} onChange={(e) => setCountrySearch(e.target.value)} placeholder="Cari nama negara..." className="w-full bg-transparent py-2 text-sm outline-none" disabled={!serviceId || showCountryLoading}/>
            </div>

            <div className="mt-3 grid grid-cols-2 rounded-2xl border border-[var(--yinn-border)] p-1">
              <button onClick={() => setSortMode("rate")} className={cx("rounded-xl py-2 text-sm font-extrabold", sortMode === "rate"
                ? "border border-[var(--yinn-border)] bg-[var(--yinn-surface)]"
                : "text-[var(--yinn-muted)]")}>
                Rate
              </button>
              <button onClick={() => setSortMode("harga")} className={cx("rounded-xl py-2 text-sm font-extrabold", sortMode === "harga"
                ? "border border-[var(--yinn-border)] bg-[var(--yinn-surface)]"
                : "text-[var(--yinn-muted)]")}>
                Harga
              </button>
            ...
            <div className="mt-3">
              {!serviceId ? (<div className="rounded-2xl border border-[var(--yinn-border)] p-4 text-sm text-[var(--yinn-muted)]">
                  Pilih aplikasi dulu.
                </div>) : showCountryLoading ? (<div className="overflow-hidden rounded-2xl border border-[var(--yinn-border)]">
                  <div className="divide-y divide-[var(--yinn-border)]">
                    {Array.from({ length: 8 }).map((_, i) => (<SkeletonRowCountry key={i}/>))}
                  </div>
                </div>) : (<div className="rounded-2xl border border-[var(--yinn-border)]">
                  <div className="divide-y divide-[var(--yinn-border)]">
                    ...
                    return (<div key={String(c?.number_id || Math.random())}>
                          <Reveal>
                            <button className="flex w-full items-center gap-3 p-3 text-left hover:bg-black/5 dark:hover:bg-white/5" onClick={() => {
                            setExpandedCountryId(open ? "" : String(c?.number_id || ""));
                              ...
                              <div className="grid h-10 w-10 place-items-center overflow-hidden rounded-xl border border-[var(--yinn-border)] bg-black/5 dark:bg-white/5">
                                {flagUrl ? (<img src={flagUrl} alt={c?.name || "Flag"} className="h-6 w-6" loading="lazy"/>) : (<div className="h-6 w-6 rounded-md bg-black/10 dark:bg-white/10"/>)}
                              </div>
                            ...
                            : "—"}
                              ...
                              <ChevronRight size={18} className={cx("text-[var(--yinn-muted)] transition", open ? "rotate-90" : "")}/>
                            </button>
                          </Reveal>

                          {open && (<div className="px-3 pb-3">
                              {pricelist.length === 0 ? (<div className="rounded-2xl border border-[var(--yinn-border)] p-3 text-xs text-[var(--yinn-muted)]">
                                  Provider kosong / stok habis untuk negara ini.
                                </div>) : (<div className="overflow-hidden rounded-2xl border border-[var(--yinn-border)]">
                                  <div className="divide-y divide-[var(--yinn-border)]">
                                    {modalKickLoading ? (<>
                                        <SkeletonProviderRow />
                                        <SkeletonProviderRow />
                                    ...
                                    return (<Reveal key={pid}>
                                            <div className="relative">
                                              {loading ? (<div className="absolute inset-0 z-10 overflow-hidden rounded-2xl">
                                            ...
                                            p?.rate !== null ? (<span className="rounded-full bg-zinc-500/10 px-2 py-1 text-[11px] font-extrabold text-zinc-600">
                                                ...
                                                <div className="ms-auto flex items-center gap-2">
                                                  ...
                                                  <button onClick={() => openOperatorPicker(c, p)} className="rounded-xl border border-[var(--yinn-border)] px-4 py-2 text-xs font-extrabold" disabled={!!orderingKey}>
                                                    Order
                                                  </button>
                                                </div>
                                              </div>
                    ...
                    {!filteredCountries.length && (<div className="p-4 text-sm text-[var(--yinn-muted)]">
      ...
      {/* OPERATOR MODAL */}
      <Modal open={openOperator} onClose={() => {
            if (opOrdering)
                return;
            setOpenOperator(false);
        ...
        {opLoading ? (<div className="grid gap-3">
            <div className="rounded-2xl border border-[var(--yinn-border)] p-4">
              ...
              <div className="mt-3 grid grid-cols-4 gap-3">
                {Array.from({ length: 8 }).map((_, i) => (<div key={i} className="rounded-2xl border border-[var(--yinn-border)] p-3">
            ...
            <div className="mt-3 grid grid-cols-4 gap-3">
              {operators.map((op) => {
                ...
                return (<button key={id || name} disabled={opOrdering} onClick={() => {
                    ...
                    }} className="rounded-2xl border border-[var(--yinn-border)] p-3 text-center hover:bg-black/5 dark:hover:bg-white/5 disabled:opacity-60" style={{ boxShadow: "var(--yinn-soft)" }}>
                    <div className="mx-auto grid h-12 w-12 place-items-center overflow-hidden rounded-2xl border border-[var(--yinn-border)] bg-black/5 dark:bg-white/5">
                      {img ? (<img src={img} alt={name} className="h-10 w-10 object-contain" loading="lazy"/>) : (<div className="text-sm font-extrabold">OP</div>)}
                    </div>
                    <div className="mt-2 truncate text-xs font-extrabold">
                      {name}
                    </div>
            ...
            <button onClick={() => setOpenOperator(false)} disabled={opOrdering} className="mt-4 w-full rounded-2xl border border-[var(--yinn-border)] py-3 text-sm font-extrabold disabled:opacity-60">
