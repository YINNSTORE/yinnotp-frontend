...
/**
 * ================= BACKEND WALLET API =================
 * Endpoint /api/wallet/* harus proxy ke backend wallet kamu.
 */
const API_WALLET = {
...
/**
 * Backend wallet lu nge-guard pakai Authorization: Bearer <token>.
 * Kita coba ambil token dari beberapa key localStorage/sessionStorage yang umum.
 * (Kalau key lu beda, tinggal tambahin).
 */
  ...
  for (const k of keys) {
    try {
      ...
      if (s.startsWith("{") && s.endsWith("}")) {
        try {
          const obj = JSON.parse(s);
          const n = obj?.name || obj?.username || obj?.user?.name || obj?.user?.username;
...
function errMsg(defaultMsg, r) {
  ...
  if (
    ...
    return "Saldo provider (YinnOTP) tidak cukup";
  }
...
/* ============== mini accordion (UI only) ============== */
function FaqItem({ icon, title, children, open, onToggle }) {
  return (
    <div className="rounded-2xl border border-[var(--yinn-border)] bg-black/5 p-0 dark:bg-white/5">
      <button
        type="button"
        onClick={onToggle}
        className="flex w-full items-center gap-3 px-4 py-3 text-left"
      >
        <span className="grid h-9 w-9 place-items-center rounded-xl border border-[var(--yinn-border)] bg-[var(--yinn-surface)]">
          {icon}
        </span>
        <div className="min-w-0 flex-1">
        ...
        <ChevronRight
          size={18}
          className={cx(
            "text-[var(--yinn-muted)] transition-transform",
            open ? "rotate-90" : ""
          )}
        />
      </button>
      {open ? (
        <div className="px-4 pb-4 text-[13px] leading-relaxed text-[var(--yinn-muted)]">
          {children}
        </div>
      ) : null}
    </div>
...
/* ============== floating support (UI only) ============== */
function FloatingSupport() {
  const [open, setOpen] = useState(false);
  const [tab, setTab] = useState("support"); // support | channel | bot

  const TabBtn = ({ id, icon, label }) => (
    <button
      onClick={() => setTab(id)}
      className={cx(
        "flex-1 rounded-xl border px-2 py-2 text-xs font-extrabold transition",
        tab === id
          ? "border-blue-500/40 bg-blue-500/10 text-blue-600"
          : "border-[var(--yinn-border)] bg-[var(--yinn-surface)] text-[var(--yinn-muted)]"
      )}
      type="button"
    >
  ...
  return (
    <div className="fixed bottom-[110px] right-4 z-[60]">
      {open ? (
        <div
          className="mb-3 w-[320px] max-w-[86vw] overflow-hidden rounded-3xl border border-[var(--yinn-border)] bg-[var(--yinn-surface)] shadow-[0_18px_55px_rgba(0,0,0,0.22)]"
          style={{ boxShadow: "var(--yinn-soft)" }}
        >
          <div className="border-b border-[var(--yinn-border)] p-4">
            ...
            <div className="mt-3 flex gap-2">
              ...
              <TabBtn
                ...
                label="Channel"
              />
              <TabBtn id="bot" icon={<Bot size={14} />} label="Bot" />
            </div>
          </div>
          ...
          <div className="p-4">
            {tab === "support" ? (
              <div className="grid gap-2">
                <a
                  href="https://t.me/rumahotp_com"
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-3 rounded-2xl border border-[var(--yinn-border)] bg-black/5 p-3 dark:bg-white/5"
                >
                  <span className="grid h-10 w-10 place-items-center rounded-2xl bg-blue-500/15 text-blue-600">
                    <Send size={18} />
                  </span>
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm font-extrabold">
                  ...
                  <ExternalLink size={16} className="text-[var(--yinn-muted)]" />
                </a>

                <div className="rounded-2xl border border-[var(--yinn-border)] bg-black/5 p-3 text-xs text-[var(--yinn-muted)] dark:bg-white/5">
                  Jam aktif: 09:00 - 21:00 (kecuali Kamis/Jumat)
                </div>
              </div>
            ) : tab === "channel" ? (
              <div className="grid gap-2">
                <a
                  href="https://t.me/inforumahotp"
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-3 rounded-2xl border border-[var(--yinn-border)] bg-black/5 p-3 dark:bg-white/5"
                >
                  <span className="grid h-10 w-10 place-items-center rounded-2xl bg-emerald-500/15 text-emerald-600">
                    <Users size={18} />
                  </span>
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm font-extrabold">
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="grid h-14 w-14 place-items-center rounded-2xl border border-[var(--yinn-border)] text-white"
        style={{
          background:
...
export default function OrderPage() {
  ...
  function extractBalanceFromJson(j) {
    ...
    if (!hasBalanceFieldInJson(j)) return null;
  ...
  function isWalletSuccess(r) {
    ...
    if (typeof flag === "string") {
      ...
      if (s === "ok" || s === "success" || s === "true" || s === "1") return true;
      if (s === "fail" || s === "failed" || s === "error" || s === "false" || s === "0") return false;
    }

    if (typeof j.error === "string" && j.error.trim()) return false;
  ...
  async function loadBalance(opts) {
    ...
    try {
      ...
      if (b == null) {
        ...
        const cached = readBalanceCache();
        if (cached > 0) setBalanceIDR(cached);
        if (!silent) toast.error("Gagal load saldo");
        return;
      }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (cb != null && cb > 0) setBalanceIDR(cb);
    } catch {}
  ...
  const headerName = userName || "YinnOTP";
  const headerSub = `${greeting}${userName ? `, ${userName}` : ""}`;
      ...
      <main className="mx-auto max-w-[520px] px-4 pt-4 pb-[calc(120px+env(safe-area-inset-bottom))]">
        <section className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div
              className="grid h-11 w-11 place-items-center overflow-hidden rounded-full border border-[var(--yinn-border)] bg-black/5 dark:bg-white/5"
              style={{ boxShadow: "var(--yinn-soft)" }}
            >
              <User size={18} className="text-[var(--yinn-muted)]" />
            </div>
            <div className="min-w-0">
              <div className="truncate text-sm font-extrabold">{headerName}</div>
              <div className="truncate text-xs text-[var(--yinn-muted)]">
                {headerSub}
              </div>
            </div>
          ...
          <button
            ...
            className="grid h-11 w-11 place-items-center rounded-2xl border border-[var(--yinn-border)] bg-[var(--yinn-surface)]"
            style={{ boxShadow: "var(--yinn-soft)" }}
            title="Profile"
            aria-label="Profile"
          >
        ...
        <section className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="rounded-3xl border p-4" style={cardStyle}>
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, rgba(242,110,90,0.95), rgba(245,138,61,0.95), rgba(89,192,166,0.95))",
                  }}
                >
                  <Wallet size={18} />
                </div>
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <span className="text-xs font-extrabold text-[var(--yinn-muted)]">
                <MsBadge ms={latencyMs} /> latensi
              </span>

              <button
                onClick={() => loadBalance({ silent: false })}
                className="ms-auto inline-flex items-center gap-2 rounded-full border border-[var(--yinn-border)] bg-[var(--yinn-surface)] px-3 py-1 text-xs font-extrabold text-[var(--yinn-muted)]"
                title="Refresh saldo"
              >
                <RefreshCw size={14} />
                Refresh
          ...
          <div
            className="rounded-3xl border p-4 text-white"
            style={{
              borderColor: "var(--yinn-border)",
              boxShadow: "var(--yinn-soft)",
              background:
          ...
          >
            ...
            <div className="mt-4 flex items-center justify-between">
              <div className="flex -space-x-2">
                {popularResolved.slice(0, 4).map((p) => (
                  <div
                    key={p.name}
                    className="grid h-10 w-10 place-items-center rounded-full border border-white/25 bg-white/15 backdrop-blur"
                    style={{ boxShadow: "0 10px 25px rgba(0,0,0,0.18)" }}
                  >
                    <img
                      src={p.img}
                      alt={p.name}
                ...
                <div className="grid h-10 w-10 place-items-center rounded-full border border-white/25 bg-white/15 text-xs font-extrabold">
                  +99
                </div>
              </div>
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-2xl bg-white/15 px-4 py-3 text-sm font-extrabold"
              >
        ...
        <section className="mt-4 rounded-3xl border p-4 md:hidden" style={cardStyle}>
          ...
          {!activeOrder ? (
            <div className="grid place-items-center rounded-2xl border border-[var(--yinn-border)] p-6 text-center">
            ...
            (() => {
              ...
              return (
                <div className="rounded-2xl border border-[var(--yinn-border)] bg-white/40 p-4 dark:bg-black/10">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex min-w-0 items-center gap-3">
                        <div className="grid h-12 w-12 place-items-center overflow-hidden rounded-2xl border border-[var(--yinn-border)] bg-black/5 dark:bg-white/5">
                          {appImg ? (
                            <img src={appImg} alt={appName} className="h-9 w-9" loading="lazy" />
                          ) : (
                        ...
                        <div className="min-w-0">
                          <div className="truncate text-base font-extrabold">{appName}</div>
                        </div>
                      </div>
        ...
        {/* Desktop pending */}
        <section className="mt-4 hidden rounded-3xl border p-4 md:block" style={cardStyle}>
          <div className="mb-3 flex items-center justify-between">
            <div className="text-sm font-extrabold">Pesanan Pending</div>
            <button
              onClick={() => activeOrder?.order_id && startPolling(activeOrder.order_id)}
          ...
          <div className="overflow-hidden rounded-2xl border border-[var(--yinn-border)]">
            <div className="grid grid-cols-12 gap-2 border-b border-[var(--yinn-border)] bg-black/5 px-3 py-2 text-[11px] font-extrabold text-[var(--yinn-muted)] dark:bg-white/5">
              <div className="col-span-2">Layanan</div>
              <div className="col-span-3">Nomor</div>
            ...
            {!activeOrder ? (
              ...
              <div className="grid grid-cols-12 gap-2 px-3 py-3 text-sm">
                <div className="col-span-2 font-extrabold">
                  {String(activeOrder.service || "—")}
                </div>

                <div className="col-span-3">
                  <div className="flex items-center gap-2">
                    <div className="truncate font-extrabold">
                      {String(activeOrder.phone_number || "—")}
                    </div>
                    <button
                      onClick={() => {
                        const phone = String(activeOrder.phone_number || "");
                      ...
                      className="grid h-8 w-8 place-items-center rounded-xl border border-[var(--yinn-border)]"
                      title="Copy"
                    >
                      <Copy size={14} className="text-[var(--yinn-muted)]" />
                    </button>
                  </div>
                  <div className="mt-1 text-[11px] text-[var(--yinn-muted)]">
                    Operator: {String(activeOrder.operator || "any")}
                  </div>
                </div>

                <div className="col-span-2 font-extrabold">
                  {formatIDR(activeOrder.price || 0)}
                </div>

                <div className="col-span-2 text-center font-extrabold text-amber-600">
                  {statusLabel(activeOrder.status)}
                </div>

                <div className="col-span-1 text-center font-extrabold">
                  {String(activeOrder.otp_code || "-").trim() ? (
                    <span className="inline-flex rounded-full bg-emerald-500/10 px-2 py-1 text-[11px] text-emerald-600">
                      {String(activeOrder.otp_code || "-").trim()}
                    </span>
                  ) : (
                    <span className="text-[var(--yinn-muted)]">—</span>
                  )}
                </div>
                ...
                <div className="col-span-2 flex justify-end gap-2">
                  ...
                  <button
                    onClick={() => setStatus("cancel")}
                    disabled={!canCancel}
                    className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-extrabold disabled:opacity-50"
                    style={{
                      borderColor: "rgba(239,68,68,0.45)",
                      color: "rgb(239,68,68)",
                      background: "rgba(239,68,68,0.06)",
                    }}
                    title={canCancel ? "Batalkan" : `Tunggu ${mmssLeft(leftCancel)}`}
                  >
                    <XCircle size={14} />
                    Batal
                  </button>
                ...
                <div className="col-span-12 mt-2 text-[11px] text-[var(--yinn-muted)]">
                  {canCancel ? (
                    <span>Sudah bisa klik batal.</span>
                  ) : (
                    <span>
                                            ...
                                            <div className="relative">
                                              ...
                                              <div className="flex items-center gap-2 p-3">
                                                ...
                                                <div className="ms-auto flex items-center gap-2">
                                                  ...
                                                  <button
                                                    onClick={() => openOperatorPicker(c, p)}
...
/* FAQ card as component to keep OrderPage readable */
function FaqCard() {
  ...
  return (
    <div className="rounded-3xl border p-4" style={cardStyle}>
      ...
      <div className="mt-3 grid gap-2">
        ...
        >
          Baca info sebelum deposit atau membeli. Kalau sedang maintenance, berarti
          lagi perbaikan. Kalau order kosong, berarti stok habis dan belum
          restock.
        </FaqItem>
        ...
        >
          ...
          Beberapa aplikasi (mis. WhatsApp/Telegram) ketat—kadang perlu VPN.
        </FaqItem>
        ...
        >
          Refund berlaku kalau OTP belum diterima sama sekali. Kalau perlu, hubungi
          support dan sertakan ID order.
        </FaqItem>
        ...
        >
          Nomor akan auto-expire. Sistem akan meng-handle status expired, dan
          refund mengikuti aturan (OTP harus kosong).
        </FaqItem>
        ...
        >
          Refund ketika belum menerima OTP sama sekali. Jika OTP sudah diterima,
          refund tidak berlaku.
        </FaqItem>
      </div>
    </div>
  );
