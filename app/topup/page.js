...
function statusLabel(s) {
  ...
  if (v.includes("success") || v.includes("completed") || v.includes("paid"))
    return "Sukses";
          ...
          return (
            <div key={s} className="flex-1">
              <div className="flex items-center">
                <div
                  ...
                  style={{
                    ...
                    color:
                      done || active ? "var(--yinn-text)" : "var(--yinn-muted)",
                    fontWeight: 800,
                  }}
                >
                  {i}
              ...
              <div
                ...
                style={{
                  color: active ? "var(--yinn-text)" : "var(--yinn-muted)",
                }}
              >
                {s}
              </div>
...
function CopyButton({ text }) {
  ...
  return (
    <button
      ...
      className="text-[11px] font-bold text-[var(--yinn-muted)]"
    >
...
export default function TopupPage() {
  ...
  function loadLocal(uid) {
    const bal =
      Number(String(localStorage.getItem(balanceKey(uid)) || "0").replace(/[^\d]/g, "")) ||
      0;
    ...
    const existingIds = new Set(
      histArr.map((x) => String(x?.order_id || x?.id || ""))
    );
    const merged = [
      ...pendingArr.filter(
        (p) => !existingIds.has(String(p?.order_id || ""))
      ),
      ...histArr,
    ];
    ...
    try {
      ...
      try {
        ...
        const serverIds = new Set(
          (j.history || []).map((x) => String(x?.order_id || x?.id || ""))
        );
        const nextPending = pendingArr.filter(
          (p) => !serverIds.has(String(p?.order_id || ""))
        );
  ...
  const goNext = () => setStep((s) => clamp(s + 1, 1, 2));
  const goBack = () => setStep((s) => clamp(s - 1, 1, 2));
  ...
  function addPendingLocal(uid, item) {
    try {
      ...
      const next = [
        ...
        ...list.filter(
          (x) =>
            String(x?.order_id || "") !== String(item?.order_id || "")
        ),
      ];
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      setOpen(false);
      router.push(
        `/topup/pay?method=qris&amount=${a}&order_id=${encodeURIComponent(
          order_id
        )}`
      );
    } finally {
                  ...
                  style={{
                    borderColor:
                      amount === p
                        ...
                        : "var(--yinn-border)",
                    background:
                      amount === p ? "rgba(67,97,238,.10)" : "transparent",
                  }}
                >
              ...
              <div className="mt-2 flex items-center gap-2 rounded-xl border border-[var(--yinn-border)] px-3 py-3">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setAmount(
                      raw ? clamp(Number(raw), 2000, 1000000) : 2000
                    );
                  }}
                  inputMode="numeric"
          ...
          <div className="mt-4">
            <div className="rounded-2xl border border-[var(--yinn-border)] p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="text-sm font-extrabold">QRIS</div>
                  <div className="text-xs text-[var(--yinn-muted)]">
                    Pembayaran instan
                  </div>
                </div>
              ...
              <div className="mt-3 text-sm">
                ...
                <div className="mt-2 flex items-center justify-between">
                  ...
                  <span className="font-extrabold">qris</span>
                </div>
